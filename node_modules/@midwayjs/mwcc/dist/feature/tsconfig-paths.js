    ...
    transform(ctx) {
        ...
        return {
            ...
            ImportType(node) {
                ...
                Object.assign(node, {
                    argument: factory.createLiteralTypeNode(factory.createStringLiteral(target)),
                });
                UpdateParent(node.argument, node);
                UpdateParent(node.argument.literal, node.argument);
                return node;
            },
            // export xxx from '...'
            ...
            CallExpression(node) {
                ...
                updateSourceFileResolvedModules(node.getSourceFile(), target, moduleSpecifier);
                const argument = factory.createStringLiteral(target);
                const newNode = factory.updateCallExpression(node, node.expression, node.typeArguments, [argument]);
                newNode.flags = node.flags;
                UpdateParent(newNode, node.parent);
                UpdateParent(argument, node);
                return newNode;
            },
        };
    },
...
function matchAliasPath(moduleSpecifier, sourceFilePath, compilerOptions) {
    ...
    let target = (0, util_1.toUnix)(path_1.default.normalize(path_1.default.relative(sourceFileDir, resolvedModule.resolvedFileName)));
...
function updateImportExportDeclaration(node, sourceFilePath, compilerOptions) {
     ...
     */
    const newModuleSpecifier = updateWithOriginal(typescript_1.default.factory.createStringLiteral(target), node.moduleSpecifier);
    Object.assign(node, {
...
//
function UpdateParent(currentNode, parent) {
