...
function createTransformer(host, checker, config) {
    ...
    return ctx => {
        const transformCtx = (0, transformation_context_1.createTransformationContext)(ctx, checker);
        const pipeline = transformers.map(transformer => {
            const it = transformer.transform(transformCtx);
            if (typeof it === 'object') {
                return (0, util_1.chainBundle)(conditionalTransform(it, transformCtx));
            }
            return (0, util_1.chainBundle)(it);
        });
    ...
    function conditionalTransform(map, ctx) {
        return (node) => {
            ...
            let result = Object.keys(map).reduce((sourceFile, pattern) => {
                return (0, query_1.visitMatch)(sourceFile, (0, query_1.parse)(pattern), map[pattern], ctx);
            }, node);
